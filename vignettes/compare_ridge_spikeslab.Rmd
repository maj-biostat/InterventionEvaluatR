...
```{r viewdata}
   ...
   sa3<-sa2[sa2$age %in% unique(sa2$age)[1:3],]
...
Adding more samples does not affect results at all.
```{r}
#source('inla_functions.R')
#results1<-evaluatr.impact(analysis, variants=c('full', 'time', 'time_no_offset') )

ptm <- proc.time()
results.ridge<-evaluatr.impact(analysis.ridge, variants=c('full', 'time', 'time_no_offset') )
proc.time() - ptm

# 4 minutes with 5000 samples
 ptm <- proc.time()
 results.ridge.iid<-evaluatr.impact(analysis.ridge, variants=c('full', 'time', 'time_no_offset') )
 proc.time() - ptm
```
...
compare.results(results.ridge)
compare.results(results.ridge.iid)

#compare.results(results.ridge.rep)
...
How much of the variability is explained by fixed vs random effects?
```{r, fig.width=9, fig.height=4}
par(mfrow=c(1,2))
for(i in names(results.ridge$full$groups)){
  rangey<-range(c(results.ridge$full$groups[[i]]$rand.eff.combined.q,results.ridge$full$groups[[i]]$fixed.effect.hdi ))
matplot(results.ridge$full$groups[[i]]$rand.eff.combined.q,type='l', lty=c(2,1,2), col='black', bty='l', main="", ylab='Random', ylim=rangey)
abline(h=0)
matplot(results.ridge$full$groups[[i]]$fixed.effect.hdi,type='l', lty=c(1,2,2), col='black', bty='l', main="", ylab='Fixed',ylim=rangey)
abline(h=0)
}
```
